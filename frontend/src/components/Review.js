...
const Review = ({ onReviewUpdate }) => {
  ...
  useEffect(() => {
    ...
    axios
      .get(`http://localhost:5000/api/socialcard/reviews/${localStorage.getItem("id")}/${bookId}`)
      .then((response) => {
   ...
   const fetchBooksWithReviews = async () => {
    try {
      const response = await axios.get(
        `http://localhost:5000/api/socialcard/reviews/${localStorage.getItem("id")}/${bookId}`
      );
  ...
  const handleSubmit = (e) => {
    ...
    const apiCall = existingReview
      ? axios.put(`http://localhost:5000/api/socialcard/reviews/${existingReview._id}`, {
          review: reviewText,
          rating,
          userId: localStorage.getItem('id'),
        })
      : axios.post('http://localhost:5000/api/socialcard/reviews', {
  ...
  const handleDelete = () => {
    if (existingReview) {
      axios
        .delete(`http://localhost:5000/api/socialcard/reviews/${existingReview._id}`, {
          data: { userId: localStorage.getItem('id') },
        })
        .then(() => {
          alert('Review deleted successfully!');
          if (onReviewUpdate) onReviewUpdate(); // Notify MyBooks page
          navigate('/'); // Navigate back to MyBooks page
        })
        .catch((error) => {
