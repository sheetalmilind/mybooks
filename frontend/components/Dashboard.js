...
const Dashboard = () => {
  ...
  useEffect(() => {
    const fetchSearchResults = async () => {
      ...
      try {
        const response = await fetch(`${SERVER_URL}/api/books/search?query=${query}`);
   ...
   const addToCurrentlyReading = (book) => {
    ...
    useEffect(() => {
      const fetchReviews = async () => {
        try {
          const response = await fetch(`${SERVER_URL}/api/socialcard/dashboard-reviews/${localStorage.getItem("id")}`);
          //, {
          //  headers: {
 ...
 // useEffect(() => {
    const fetchCurrentlyReading = async (page) => {
      ...
      try {
        ...
        const response = await fetch(`${SERVER_URL}/api/books/currently-reading/${userId}`);
        const data = await response.json();
        setItems(data.items);
            ...
            data.books.map(async (book) => {
              try {
                const progressResponse = await fetch(
                  `${SERVER_URL}/api/books/progress/${userId}/${book._id}` // Use book._id as the book ID
                );
