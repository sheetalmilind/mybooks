...
const MyBooks = () => {
  ...
  const fetchBooksWithReviews = async () => {
    try {
      const response = await axios.get(
        `http://localhost:5000/api/books/get-books-with-reviews/${localStorage.getItem("id")}`
      );
 ...
 const handleWriteReview = (bookId) => {
  ...
  const handleRatingChange = async (bookId, rating) => {
    try {
      await axios.patch(
        `http://localhost:5000/api/socialcard/reviews/update-rating/${bookId}`,
        {
          rating,
        }
      );
  ...
  // Add book to "Currently Reading" and clear search
  const handleAddToCurrentlyReading = async (book) => {
    ...
    try {
      const response = await fetch('http://localhost:5000/api/books/addBookUser', {
        method: 'POST',
      ...
      if (response.ok && data.book) {
        ...
        setBooksWithReviews((prevBooks) => [...prevBooks, normalizedBook]);
  ...
  return (
    <div className="my-books-container">
      ...
      {/* Home Button */}
      <Link to="/dashboard" className="home-link">Home</Link>
