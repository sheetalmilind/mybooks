...
const UpdateProgressModal = ({ book, onClose, onProgressUpdated }) => {
  ...
  const handleSaveProgress = async () => {
    ...
    try {
      const response = await fetch(`http://localhost:5000/api/books/update-progress/${userId}/${book._id}`, {
        method: 'PUT',
  ...
  const fetchBookProgress = async (userId, bookId) => {
    try {
      const response = await fetch(`http://localhost:5000/api/books/progress/${userId}/${bookId}`);
      const data = await response.json();
