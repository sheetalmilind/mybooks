...
const MyBooks = () => {
  ...
  const fetchBooksWithReviews = async (page) => {
    try {
      const response = await axios.get(
        `${SERVER_URL}/api/books/get-books-with-reviews/${localStorage.getItem("id")}`
      );
  ...
  useEffect(() => {
    const fetchSearchResults = async () => {
      ...
      try {
        const response = await axios.get(
          `${SERVER_URL}/api/books/search?query=${query}`
        );
 ...
 const handleWriteReview = (bookId) => {
  ...
  const handleRatingChange = async (bookId, rating) => {
    try {
      await axios.patch(
        `${SERVER_URL}/api/socialcard/reviews/update-rating/${bookId}`,
        {
          rating,
        }
      );
