...
const Review = ({ onReviewUpdate }) => {
  ...
  useEffect(() => {
    ...
    axios
      .get(`${SERVER_URL}/api/socialcard/reviews/${localStorage.getItem("id")}/${bookId}`)
      .then((response) => {
  ...
  const handleSubmit = (e) => {
    ...
    const apiCall = existingReview
      ? axios.put(`${SERVER_URL}/api/socialcard/reviews/${existingReview._id}`, {
          review: reviewText,
          rating,
  ...
  const handleDelete = () => {
    if (existingReview) {
      axios
        .delete(`${SERVER_URL}/api/socialcard/reviews/${existingReview._id}`, {
