...
const Dashboard = () => {
   ...
   const addToCurrentlyReading = (book) => {
    ...
    useEffect(() => {
      const fetchReviews = async () => {
        try {
          const response = await fetch(`http://localhost:5000/api/socialcard/dashboard-reviews/${localStorage.getItem("id")}`);
          //, {
          //  headers: {
 ...
 // useEffect(() => {
    const fetchCurrentlyReading = async () => {
      ...
      try {
        const response = await fetch(`http://localhost:5000/api/books/currently-reading/${userId}`);
        const data = await response.json();
            ...
            data.books.map(async (book) => {
              try {
                const progressResponse = await fetch(
                  `http://localhost:5000/api/books/progress/${userId}/${book._id}` // Use book._id as the book ID
                );
